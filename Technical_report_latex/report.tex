...
\begin{center}
    ...
    \fbox{\parbox{0.8\textwidth}{
        ...
        \vspace{0.5cm}
        \textbf{LEVEL V CLEARANCE REQUIRED}
    }}
    ...
    \vspace{3cm}
    \begin{tabular}{ll}
        \textbf{PROJECT LEAD:} & S. HARI PREETHAM \\
        \textbf{SUPERVISING AUTHORITY:} & DR. A. CHENNAKESAVA REDDY \\
        \textbf{INSTITUTION:} & JNTUH UCESTH \\
        \textbf{DOCUMENT ID:} & \texttt{PRJ-ARES-V-2025-DOC-001} \\
...
Forward kinematics calculates the final laser spot position on the screen given a set of mirror angles. To model this, we first define a standard 3D coordinate system:
\begin{itemize}
    \item The origin is at the center of the first mirror (X-mirror).
    \item The \textbf{Z-axis} represents the path of the incoming laser beam.
    \item The \textbf{X-mirror} deflects the beam along the X-axis by rotating around the \textbf{Y-axis} by an angle \(\theta_y\).
    \item The \textbf{Y-mirror} deflects the beam along the Y-axis by rotating around the \textbf{X-axis} by an angle \(\theta_x\).
\end{itemize}

The initial state is an incoming beam along the Z-axis, \(\vec{V}_{in} = [0, 0, 1]^T\). The undeflected normals of the mirrors are \(\vec{N}_{x0}\) and \(\vec{N}_{y0}\), typically oriented at 45 degrees to the incoming beam.

A rotation of a mirror by its angle changes its normal vector. The new normal is found by applying a 3D rotation matrix.
\begin{itemize}
...
The final direction of the laser beam is calculated through sequential reflections:
\begin{enumerate}
    \item \textbf{First Reflection (X-mirror):} The new normal of the X-mirror is \(\vec{N}_{x} = \mathbf{R}_y(\theta_y)\vec{N}_{x0}\). The beam is reflected: \(\vec{R}_1 = \vec{V}_{in} - 2(\vec{V}_{in} \cdot \vec{N}_{x})\vec{N}_{x}\).
    \item \textbf{Second Reflection (Y-mirror):} The new normal of the Y-mirror is \(\vec{N}_{y} = \mathbf{R}_x(\theta_x)\vec{N}_{y0}\). The beam from the first mirror, \(\vec{R}_1\), is now the incoming vector for the second: \(\vec{R}_{final} = \vec{R}_1 - 2(\vec{R}_1 \cdot \vec{N}_{y})\vec{N}_{y}\).
\end{enumerate}

\subsubsection{Ray-Plane Intersection}
With the final direction vector \(\vec{R}_{final} = [R_x, R_y, R_z]^T\), we can find where it hits the screen. Assuming the second mirror is at the origin for simplicity, the parametric equation for the ray is \(\vec{P}(t) = t \cdot \vec{R}_{final}\).
The screen is a plane defined by \(z=d\). We solve for \(t\):
\begin{equation}
    t \cdot R_z = d \implies t = \frac{d}{R_z}
\end{equation}
Substituting \(t\) back gives the final coordinates \((X_f, Y_f)\):
\begin{equation}
    X_f = t \cdot R_x = d \frac{R_x}{R_z} \quad \text{and} \quad Y_f = t \cdot R_y = d \frac{R_y}{R_z}
\end{equation}
This completes the forward kinematic model. However, for control, we need the inverse.

\subsection{Inverse Kinematics: The Challenge of Target-to-Angle Mapping}

For any practical application, we must solve the inverse problem: given a desired target on the screen \((X_{target}, Y_{target})\), what are the required mirror angles \((\theta_x, \theta_y)\)? This is the inverse kinematics (IK) problem.

Forward kinematics is insufficient for real-time control, especially on a mobile platform, because:
\begin{itemize}
    \item The pivot point \([0,0,0]\) is no longer static; it is moving and vibrating.
    \item We need to solve for angles in real-time to track a target.
    \item The forward mapping is complex and non-linear, making direct analytical inversion impractical for a real-world system with mechanical imperfections and misalignments.
\end{itemize}
